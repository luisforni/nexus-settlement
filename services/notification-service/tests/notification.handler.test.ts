...
describe('handleKafkaMessage', () => {
  ...
  it('sends email on settlement.reversed', async () => {
    const msg = envelope('settlement.reversed', {
      settlement_id: 'stl_rev_001',
      amount: '200.00',
      currency: 'USD',
      user_email: 'user@example.com',
    });
    await handleKafkaMessage('nexus.settlements', msg);
    expect(mockSendEmail).toHaveBeenCalledTimes(1);
    expect(mockSendEmail.mock.calls[0]?.[0]?.subject).toContain('reversed');
    expect(mockSendSms).not.toHaveBeenCalled();
  });

  it('discards a message whose integrity_hash does not match the payload', async () => {
    const raw = JSON.stringify({
      event_id: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890',
      event_type: 'settlement.completed',
      timestamp: new Date().toISOString(),
      payload: {
        settlement_id: 'stl_tampered',
        amount: '999.00',
        currency: 'USD',
        user_email: 'attacker@example.com',
      },
      integrity_hash: 'deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef',
    });
    await handleKafkaMessage('nexus.settlements', raw);
    expect(mockSendEmail).not.toHaveBeenCalled();
    expect(mockSendSms).not.toHaveBeenCalled();
    expect(mockSendWebhook).not.toHaveBeenCalled();
  });
