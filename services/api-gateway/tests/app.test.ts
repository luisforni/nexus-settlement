...
const SETTLEMENT_ID = '00000000-0000-4000-8000-000000000099';

describe('PATCH /api/v1/settlements/:id/cancel', () => {
  it('returns 401 with no Authorization header', async () => {
    const res = await request(app).patch(`/api/v1/settlements/${SETTLEMENT_ID}/cancel`);
    expect(res.status).toBe(401);
  });

  it('returns 403 without settlement:write scope', async () => {
    const token = makeToken(['settlement:read']);
    const res = await request(app)
      .patch(`/api/v1/settlements/${SETTLEMENT_ID}/cancel`)
      .set('Authorization', `Bearer ${token}`)
      .set('Idempotency-Key', VALID_IDEMPOTENCY_KEY);
    expect(res.status).toBe(403);
  });
  ...
  it('passes through with valid token and Idempotency-Key (not 4xx auth error)', async () => {
    const token = makeToken(['settlement:write']);
    const res = await request(app)
      .patch(`/api/v1/settlements/${SETTLEMENT_ID}/cancel`)
      .set('Authorization', `Bearer ${token}`)
...
describe('PATCH /api/v1/settlements/:id/reverse', () => {
  it('returns 401 with no Authorization header', async () => {
    const res = await request(app).patch(`/api/v1/settlements/${SETTLEMENT_ID}/reverse`);
    expect(res.status).toBe(401);
  });
