...
@pytest.fixture
def mock_fraud() -> FraudClient:

    client = AsyncMock(spec=FraudClient)
    client.score = AsyncMock(return_value={"risk_score": 0.05, "decision": "APPROVE", "model_version": "test", "scored_at": "2026-01-01T00:00:00Z"})
    return client
...
async def client(
    ...
    def override_settlement_service() -> SettlementService:
        return SettlementService(db=db_session, kafka=mock_kafka, fraud=mock_fraud)
